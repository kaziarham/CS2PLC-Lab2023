...
piecesFromThings = [ piece for piece in things if piece.__class__ == Piece ] #TASK 5.3.(c) - list comprehension that constructs a list of all pieces that are stored (among other values) in the list in variable things
...
shortItemLenghts1 = [ item.length_secs for item in playlist1 if item.length_secs < 20.0 ] # TASK 5.3.(b) - list comprehension that constructs a list of the lengths of all items that are less than 20 seconds long from the list playlist1
